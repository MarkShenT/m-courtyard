...
<div align="center">
  <img src="screenshots/0228/training-2.gif" alt="M-Courtyard 训练演示" width="800">
</div>
...
- **AI 赋能数据处理**：可使用本地模型自动将非结构化文档转化为高质量指令数据集；如果不希望使用 AI，也可退回内置规则流程。
...
- **`Ollama` 当前是可选本地运行时**：主要用于基于 Ollama 的 AI 数据集生成与一键导出到 Ollama。
- **`LM Studio` 当前已可通过 MLX 导出衔接使用**：导出的 MLX 模型今天就可以在 LM Studio 中加载。
- **`LM Studio` 的应用内一体化接入计划在 `v0.5.0` 落地**：届时会补齐模型发现、下载与本地 OpenAI 兼容服务接入，作为与 Ollama 并列的本地运行时路径，而不是继续形成 “只有 Ollama 才能接入” 的误解。
...
<div align="center">
  <img src="screenshots/0228/data-prep-1.png" alt="数据准备 1" width="48%">
  &nbsp;
  <img src="screenshots/0228/data-prep-2.gif" alt="数据准备 2" width="48%">
</div>

### 2. 模型训练
基于 Apple MLX 驱动，实时呈现 Loss 曲线、预计完成时间和训练进度。
<div align="center">
  <img src="screenshots/0228/training-1.gif" alt="训练进度 1" width="48%">
  &nbsp;
  <img src="screenshots/0228/training-3.png" alt="训练总结" width="48%">
</div>

### 3. 测试与导出
通过内置聊天界面即时验证微调后的适配器质量，并导出到 Ollama 或导出为供 LM Studio / 本地 MLX 工作流使用的 MLX 资产。
<div align="center">
  <img src="screenshots/0228/test-model.png" alt="测试模型" width="48%">
  &nbsp;
  <img src="screenshots/0228/export-2.gif" alt="导出至 Ollama" width="48%">
</div>
...
- **可选本地运行时**: 若你今天想在 LM Studio 中加载导出的 MLX 模型，可安装 [LM Studio](https://lmstudio.ai)；应用内的一体化 LM Studio 接入计划在 `v0.5.0` 提供
